...
import { ConnectionWarning } from '@/app/components/ConnectionWarning/ConnectionWarning';
...
export default async function Home() {
  const { trips, users, dbError } = await getHomePageData();
  ...
  return (
    <div className='flex flex-col flex-1 items-center justify-center font-sans'>
      <main className='flex flex-1 w-full max-w-3xl flex-col items-center justify-between py-32 px-16 sm:items-start'>
        ...
        {/*
         * Если БД недоступна — не падаем, а показываем предупреждение.
         * Данные (trips/users) в этом случае пустые.
         */}
        {dbError && <ConnectionWarning />}
        ...
        {!dbError && <AddTripSection users={users} />}

        {!dbError && <UserSection users={users} />}
      </main>
    </div>
  );
}
